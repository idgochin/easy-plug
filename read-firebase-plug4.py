...
# Balance file path
BALANCE_FILE_PATH = "balance-plug4.json"
...
def is_plug1_running():
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            if proc.info['cmdline'] and "plug4.py" in proc.info['cmdline']:
                print(f"plug4.py is already running with PID: {proc.info['pid']}")
                return True
            ...
            if data:
                ...
                if plug1.get('status') == 'active':
                    ...
                    if inactive_start_time is None:
                        ...
                        print(inactive_start_time)
                        print("Plug4 has been inactive for 10 minutes. Ending script.")
                        return
                    print(time.time() - inactive_start_time)
            time.sleep(5)

                
        print("Activating Relay 4 and LED 1 for EZP000104")
        subprocess.run(['python3', 'delete-pay.py'])

        # Check if plug1.py is already running before starting it
        if not is_plug1_running():
            subprocess.run(['python3', 'plug4.py'])
            return
        else:
            print("Skipping plug4.py execution as it is already running.")
